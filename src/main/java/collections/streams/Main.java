...
//      ----------------------FILTER---------------------------------------------------------------------
        ...
        System.out.println("tylko poarzyste, pętla: " + onlyEvenLoop);

        var integerStream = integers.stream();
        var onlyEvenStream = integerStream.filter(integer -> integer % 2 == 0).toList();
        System.out.println("Tylko pazyste stream: " + onlyEvenStream);
...
//        ------------------------MAP---------------------------------------------------------------------
        ...
        var stringsFromStream = integersToMap.stream().map(integer -> integer.toString()).toList();
        ...
        //wyciągamy wszystkie imioma osób ze szkoleniach
